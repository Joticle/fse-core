...
13. **Query Artifact Discipline.** Schema changes are explicit, human-authored, versioned artifacts stored in a single canonical location; the framework or ORM never generates, applies, or infers schema implicitly. Data queries are centralized and named as addressable artifacts, never embedded inline at the call site. Queries that cross a module or bounded-context boundary are explicitly marked, so cross-boundary access is discoverable by inspection alone. The discipline turns on that last clause: a project can keep schema and query logic legible and still let cross-boundary reads hide in plain sight, and a seam that cannot be grepped cannot be governed. Concrete file locations, naming grammar, language bindings, and boundary-marking syntax are defined by each project's binding layer or stack extension; Core defines the principle, not the stack.
